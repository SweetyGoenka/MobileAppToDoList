/**
 * Created by sgoen on 13-11-2016.
 */
...
todo.addItemPrompt = function(){
    ons.notification.prompt('Insert new to-do item label.', {
        ...
        callback: function(label){
            if(label == '' || label == null) {
                ons.notification.alert('Please try agin by inserting the to-do item label!');
                return;
            }

            if(todoStorage.add(label)) {
                ons.notification.alert('Successfully added the item to the todo list!');
                this.refresh();
            } else {
